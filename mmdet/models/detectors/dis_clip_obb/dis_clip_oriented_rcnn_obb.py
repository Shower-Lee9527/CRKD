...
class DisCLIPOrientedRCNN(OBBTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(DisCLIPOrientedRCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        # x = self.extract_feat_backbone(img)
        # from mmengine.visualization import Visualizer
        # import numpy as np
        ...
        # feature = x[0]
        # # features['output'], _ = torch.var_mean(features['output'], dim=1)
        # feature_mean, _ = torch.std_mean(feature, dim=1)
        # # features['output'] = torch.mean(features['output'], dim=1)
        # # features['output'] = torch.nanmean(features['output'], dim=1)
        # img = Image.open(img_metas[0]['filename'])
        # img = img.resize(feature.shape[-2:])
        #
        # # 将图片转换为 NumPy 数组
        # img_np = np.array(img)
        # drawn_img0 = visualizer.draw_featmap(feature_mean,  # [:, 0, :, :]
        #                                      overlaid_image=img_np,
        #                                      channel_reduction='select_max',
        #                                      # resize_shape=(1024, 1024),
        #                                      alpha=0.5)
        # visualizer.add_image('a_0', drawn_img0)
        # # for channel in range(feature.shape[1]):
        # #     drawn_img0 = visualizer.draw_featmap(feature[:, channel, :, :],  # [:, 0, :, :]
        # #                                          overlaid_image=img_np,
        # #                                          channel_reduction='select_max',
        # #                                          # resize_shape=(1024, 1024),
        # #                                          alpha=0.5)
        # #     visualizer.add_image(f'x_{channel}', drawn_img0)
        # x = self.extract_feat_neck(x)
